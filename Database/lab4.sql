...
/*Question2*/
select * from hotel where substring(hotelname,4,1)='T'

/*Question 3*/
select * from booking where dateto is null

/*Question 4*/
select guestname,guestaddress from guest where guestaddress like '%Glasgow%' and 
guestname like '%tony%'  or guestname like '%Farrel%'

/*Question 5*/
select roomno,hotelno,datefrom from booking where year(datefrom) between '2005' and '2010'
...
/* Question 10 */
SELECT hotelname FROM hotel WHERE hotelno
IN  (SELECT hotelno FROM room 
WHERE type = 'single' OR type = 'double') AND hotelno NOT IN (SELECT hotelno FROM room WHERE type = 'family')
/*Question 11*/
select * from booking where (guestno=10003 or guestno=10007) and guestno not in (10001)